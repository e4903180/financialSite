...
def generate_random_header():
    ...
    browser = random.choice(list(random_user_agents.keys()))    #key()為將dict的引號全部顯示
    user_agent = random.choice(random_user_agents[browser])
    header = copy.copy(random.choice(random_headers))   #用等於的方式是讓list_A = list_B相同記憶體位置改A會同部改B   但copy.copy() 是在不同的記憶體位置
...
def find_best_session():
    ...
    for i in range(10):
        try:
            ...
            ses.get('https://www.twse.com.tw/zh/', headers=headers, timeout=10)  #timeout 為設定嘗試連線的時間
            ses.headers.update(headers) #嘗試使用其中一組header連線，如果成功就更新requests.Session
            print('成功！')
            return ses
...
def requests_get(*args1, **args2):
    ...
    while i >= 0:
        try:
            return ses.get(*args1, **args2)    #這邊為例 *arg1 = url , **arg2 = {'headers':headers, 'verify':False}
...
def crawl_benchmark(date):
    ...
    try:
        ...
        print('**WARRN: cannot get benchmark at', date_str)
        print(e)
        return None
