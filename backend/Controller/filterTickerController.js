...
exports.filter_ticker = async function(req, res){
    let sql = 'SELECT ticker_filter.*, ticker_list.stock_num, ticker_list.stock_name, twStock.Open, twStock.High, twStock.Low, twStock.Close, twStock.Volume \
    ...
    for(let i = 0; i < conditions.length; i++){
        switch(conditions[i]){
            case "5ma突破15ma黃金交叉":
                sql += " AND ticker_filter.5ma_15ma_golden_cross=1"
                conditions[i] = "5ma_15ma_golden_cross"
                break
            case "5ma突破15ma死亡交叉":
                sql += " AND ticker_filter.5ma_15ma_death_cross=1"
                conditions[i] = "5ma_15ma_death_cross"
                break
            case "5ma突破30ma黃金交叉":
                sql += " AND ticker_filter.5ma_30ma_golden_cross=1"
                conditions[i] = "5ma_30ma_golden_cross"
                break
            case "5ma突破30ma死亡交叉":
                sql += " AND ticker_filter.5ma_30ma_death_cross=1"
